...
let log = Im.Im_main.ProofWorkerProcess.log

let main_worker options =
  let initial_vernac_state = Vernacstate.freeze_full_state () in
  try Im.Im_main.ProofWorkerProcess.main ~st:initial_vernac_state options
  with exn ->
...
let _ =
  Coqinit.init_ocaml ();
  let opts, emoptions = Coqinit.parse_arguments ~parse_extra:Im.Im_main.ProofWorkerProcess.parse_options ~usage:vsrocqtop_specific_usage () in
...
let () =
  Coqinit.init_ocaml ();
  let opts, emoptions = Coqinit.parse_arguments ~parse_extra:Im.Im_main.ProofWorkerProcess.parse_options (List.tl (Array.to_list Sys.argv)) in
